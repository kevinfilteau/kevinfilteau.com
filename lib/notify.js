...
const recap = (l) => [
    ...
    l.focus ? `La rencontre devrait porter sur : ${l.focus}` : ''
].filter(Boolean).join('\n');

export const confirmation = (l) => ({
    subject: 'C’est réservé : votre rencontre avec Kevin Filteau',
    text: `Bonjour ${l.name},

Merci. Votre rencontre d’une heure est réservée. Je vous joins dans un jour ouvrable, ${channelWord(l.channel)}, pour planifier la rencontre.
...
// To Kevin when the visitor reaches the Stripe page, before any payment.
export const notice = (l) => ({
    subject: `Nouvelle demande : ${l.name}, ${l.company}`,
    text: `${l.name} — ${l.company}
${l.email} · ${l.phone} · à joindre ${channelWord(l.channel)}
Session Stripe : ${l.sessionId}
Paiement : pas encore. Un autre courriel suit quand Stripe le confirme.

${recap(l)}`
});

// To Kevin once Stripe confirms the payment.
export const paidNotice = (l) => ({
    subject: `Paiement reçu : ${l.name}, ${l.company}`,
    text: `${l.name} — ${l.company}
${l.email} · ${l.phone} · à joindre ${channelWord(l.channel)}
Session Stripe : ${l.sessionId}
